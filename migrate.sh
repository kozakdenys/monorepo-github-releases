...
function migrateLerna() {
  ...
  npm pkg set "scripts.versionup"="lerna version"
  npm pkg set "scripts.ci:versionup:patch"="lerna version patch --no-push --no-git-tag-version --yes && npm run commit-version"
  npm pkg set "scripts.ci:versionup:minor"="lerna version minor --no-push --no-git-tag-version --yes && npm run commit-version"
  npm pkg set "scripts.ci:versionup:major"="lerna version major --no-push --no-git-tag-version --yes && npm run commit-version"
  npm pkg set "scripts.commit-version"="git add . && git commit -m \"chore(release): v\$(node -p 'require(\"./lerna.json\").version')\""
  npm pkg set "scripts.release"="lerna publish from-package"
  npm pkg set "scripts.ci:release"="lerna publish from-package --yes"
}

function migrateNpm() {
  echo_message "Migrate package.json"
  npm pkg set "scripts.ci:versionup:patch"="npm version patch --no-push --no-git-tag-version --yes && npm run commit-version"
  npm pkg set "scripts.ci:versionup:minor"="npm version minor --no-push --no-git-tag-version --yes && npm run commit-version"
  npm pkg set "scripts.ci:versionup:major"="npm version major --no-push --no-git-tag-version --yes && npm run commit-version"
  npm pkg set "scripts.commit-version"="git add . && git commit -m \"chore(release): v\$(node -p 'require(\"./package.json\").version')\""
  npm pkg set "scripts.ci:release"="npm publish --yes"
}
